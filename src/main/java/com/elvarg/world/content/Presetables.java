...
import com.elvarg.world.model.syntax.impl.ChangePresetName;
import com.elvarg.world.model.syntax.impl.CreatePreset;
...
public class Presetables {
	...
	public static void open(Player player, Presetable preset) {
		...
		if(preset != null) {
			Arrays.stream(preset.getEquipment()).filter(t -> !Objects.isNull(t) && t.isValid()).forEach(t -> player.getPacketSender().sendItemOnInterface(45044 + t.getDefinition().getEquipmentSlot(), t.getId(), t.getAmount()));
		}
	...
	private static void load(Player player, final Presetable preset) {
		...
		//Set equipment
		Arrays.stream(preset.getEquipment()).filter(t -> !Objects.isNull(t) && t.isValid()).forEach(t -> player.getEquipment().setItem(t.getDefinition().getEquipmentSlot(), t.copy()));
