...
public class Bank extends ItemContainer {
	...
	public Bank open() {
		...
		//Send configs
		getPlayer().getPacketSender().sendConfig(115, getPlayer().withdrawAsNote() ? 1 : 0).sendConfig(304, getPlayer().insertMode() ? 1 : 0).sendConfig(117, getPlayer().isSearchingBank() ? 117 : 0).sendInterfaceSet(5292, 5063);
	...
	public static void withdraw(Player player, int item, int slot, int amount, int fromBankTab) {
		...
		if(player.getStatus() == PlayerStatus.BANKING && player.getInterfaceId() == 5292) {
			...
			//Check if we're withdrawing the item from the proper tab, but only if we aren't bank searching
			...
			if(player.isSearchingBank()) {
				...
				if(!player.getBank(itemTab).contains(item)
						|| !player.getBank(BANK_SEARCH_TAB_INDEX).contains(item)) {
					return;
				}
				...
				player.getBank(itemTab).switchItem(player.getInventory(), new Item(item, amount), player.getBank(itemTab).getSlot(item), false, 
						false);
			...
			for(int i = 0; i < TOTAL_BANK_TABS; i++) {
				...
				if(b != null) {
					...
					for(Item item : b.getValidItems()) {
						addToBankSearch(player, item.copy(), false);
					}
				}
			}
	...
	/**
	 * Moves an item from one slot to another using the insert method.
	 * It will shift all other items to the right.
	 * @param player
	 * @param fromSlot
	 * @param toSlot
	 */
	...
	public Bank switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
		...
		//Check if we can hold the total amount
		if(to instanceof Inventory) {
		...
		if(getPlayer().isSearchingBank()) {
			removeFromBankSearch(getPlayer(), item.copy(), true);
		}
	...
	/**
	 * Deposits items from another container into the bank.
	 * Used for depositing inventory/equipment.
	 * @param player
	 * @param from
	 * @param ignoreReqs
	 */
	public static void depositItems(Player player, ItemContainer from, boolean ignoreReqs) {
		...
		for (Item item : from.getValidItems()) {
			from.switchItem(player.getBank(Bank.getTabForItem(player, item.getId())), item.copy(), from.getSlot(item.getId()), false, false);
		}
